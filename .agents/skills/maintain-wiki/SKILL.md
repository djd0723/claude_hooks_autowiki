...
preset. The loop runs a LIGHT, per-source lint at ingest (neighborhood-scoped); THIS is the
deep, periodic, WHOLE-WIKI sweep Karpathy describes ("periodically, ask the LLM to health-check
the wiki"). Run it over the whole graph, not just one source's neighborhood.
